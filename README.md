# BepInEx Template for Super Psycho Baseball

- [BepInEx Template for Super Psycho Baseball](#bepinex-template-for-gamename)
...
```bash
dotnet new install Boxofbiscuits97.BepInExTemplate
```
...
Once installed, the template will be available as `Super Psycho Baseball BepInEx Plugin` with an alias `spbmod`.

## Creating a Project

Open a terminal in your Super Psycho Baseball modding directory, and run:
...
```sh
dotnet new spbmod --output ModName --guid com.github.YourAccount.ModName --ts-team YourThunderstoreTeam
```
...
>
> You can run `dotnet new spbmod --help` to see all available options.
...
```sh
~/Workspace/SuperPsychoBaseball$ dotnet new spbmod --output MyCoolMod --guid com.github.Boxofbiscuits97.MyCoolMod --ts-team Boxofbiscuits97
The template "SuperPsychoBaseball BepInEx Plugin" was created successfully.

~/Workspace/SuperPsychoBaseball$ cd MyCoolMod/
~/Workspace/SuperPsychoBaseball/MyCoolMod$ tree
.
├── CHANGELOG.md
